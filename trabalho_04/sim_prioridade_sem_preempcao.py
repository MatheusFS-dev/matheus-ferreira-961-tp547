"""Simulate the non-preemptive priority queue for Trabalho 04."""
...
QUEUE_NAME = "Filas com Prioridade e sem Preempção"
QUEUE_SLUG = "prioridade_sem_preempcao"
LAMBDA_BASE = 200.0
MU_BASE = 250.0
RHO_BASE = 0.8
K_BASE = ""
...
def build_scenarios():
    """Create the common and priority-mix scenarios for this queue.

    Args:
        None: The function uses the fixed project specification values.

    Returns:
        list[dict]: Scenario dictionaries for rho and priority variations.
    ...
    return [
        ...
        {"scenario_id": "phigh_0_10", "lambda_rate": 200.0, "mu_rate": 250.0, "rho": 0.80, "K": "", "p_high": 0.10},
        {"scenario_id": "phigh_0_50", "lambda_rate": 200.0, "mu_rate": 250.0, "rho": 0.80, "K": "", "p_high": 0.50},
        {"scenario_id": "phigh_0_80", "lambda_rate": 200.0, "mu_rate": 250.0, "rho": 0.80, "K": "", "p_high": 0.80},
    ]
...
def append_time_series_until(sample_rows, next_sample_time, upper_time, scenario_id, replication, num_in_system, num_in_queue, busy_servers):
    ...
    Args:
        ...
        num_in_queue (int): Current queue length across both priority classes.
...
def choose_next_packet(high_queue, low_queue):
    """Select the next packet according to non-preemptive class priority.

    Args:
        high_queue (collections.deque[Packet]): FIFO queue for high-priority
            packets.
        low_queue (collections.deque[Packet]): FIFO queue for low-priority
            packets.

    Returns:
        Packet | None: Next packet to serve, preferring the high-priority queue
            when it is non-empty.

    Raises:
        None: Empty queues simply produce `None`.
    """
    if high_queue:
        return high_queue.popleft()
    if low_queue:
        return low_queue.popleft()
    return None
...
def build_replication_row(scenario, replication, seed, counts, queue_areas, waiting_times, system_times, service_times, waiting_times_by_class, system_times_by_class):
    """Convert one non-preemptive-priority replication into the required row.
...
def simulate_replication(scenario, replication):
    """Run one event-driven replication of the non-preemptive priority queue.

    Args:
        scenario (dict): Scenario parameters with traffic, service, and
            priority-mix values.
        replication (int): Zero-based replication index.
    ...
    Raises:
        RuntimeError: If a departure event does not match the packet in
            service.
    """
    seed = BASE_SEED + (1000 * replication) + int(scenario["lambda_rate"] * 10) + int(scenario["p_high"] * 100)
    ...
    while event_heap:
        ...
        num_in_queue = len(high_queue) + len(low_queue)
        ...
        if event_type == "arrival":
            ...
            next_packet = choose_next_packet(high_queue, low_queue)
            if next_packet is not None:
                current_packet = next_packet
                event_counter = start_service(next_packet, event_time, rng, scenario["mu_rate"], event_heap, event_counter)
...
def save_outputs(replication_rows, occupancy_rows, sample_rows, scenarios):
    """Write all required result files for the non-preemptive priority queue.
    ...
    config = {
        ...
        "base_seed": BASE_SEED,
        "scenarios": scenarios,
        "description": "Fila com duas classes, prioridade sem preempcao, buffer infinito, chegadas Poisson e servico exponencial.",
    }
...
def main():
    """Run every scenario and replication for the non-preemptive priority queue.
