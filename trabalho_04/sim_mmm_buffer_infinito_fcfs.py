"""Simulate the M/M/m queue with infinite FCFS buffer for Trabalho 04."""
...
QUEUE_NAME = "Fila M/M/m/∞/∞/∞/FCFS"
QUEUE_SLUG = "mmm_buffer_infinito_fcfs"
LAMBDA_BASE = 200.0
MU_BASE = 250.0
RHO_BASE = 0.8
K_BASE = ""
...
def build_scenarios():
    """Create the common rho scenarios for the M/M/m queue.

    Args:
        None: The function uses the fixed values from the project plan.

    Returns:
        list[dict]: Scenario dictionaries with the required traffic parameters.
...
def append_time_series_until(sample_rows, next_sample_time, upper_time, scenario_id, replication, num_in_system, num_in_queue, busy_servers):
    """Sample the piecewise-constant multi-server state at one-second steps.
    ...
    Args:
        ...
        busy_servers (int): Current number of busy servers.
...
def find_free_server(servers):
    """Return the lowest-index idle server slot.

    Args:
        servers (list[Packet | None]): Current server occupancy list.

    Returns:
        int | None: Idle server index, or `None` if all servers are busy.

    Raises:
        None: The function only scans the provided list.
    """
    for index, packet in enumerate(servers):
        if packet is None:
            return index
    return None


def start_service(packet, clock, rng, mu_rate, server_id, event_heap, event_counter):
    """Assign service to one packet on a specific server and schedule departure.
    ...
    Args:
        ...
        server_id (int): Server handling this packet.
...
def build_replication_row(scenario, replication, seed, counts, queue_areas, waiting_times, system_times, service_times, waiting_times_by_class, system_times_by_class):
    """Convert one M/M/m replication into the required CSV row.
    ...
    server_utilization = safe_divide(queue_areas["busy_server_area"], SERVERS * measured_time)
...
def simulate_replication(scenario, replication):
    """Run one event-driven replication of the infinite-buffer M/M/m queue.

    Args:
        scenario (dict): Scenario parameters with traffic and service values.
        replication (int): Zero-based replication index.
    ...
    Raises:
        RuntimeError: If a departure event does not match the packet recorded in
            its server slot.
    """
    seed = BASE_SEED + (1000 * replication) + int(scenario["lambda_rate"] * 10) + SERVERS
    ...
    while event_heap:
        event_time, _, event_type, packet_id, server_id = heapq.heappop(event_heap)
        effective_time = min(event_time, SIM_TIME)
        num_in_queue = len(queue)
        busy_servers = sum(packet is not None for packet in servers)
        num_in_system = num_in_queue + busy_servers
        ...
        if event_type == "arrival":
            ...
            free_server = find_free_server(servers)
            if free_server is None:
                queue.append(packet)
            else:
                servers[free_server] = packet
                event_counter = start_service(packet, event_time, rng, scenario["mu_rate"], free_server, event_heap, event_counter)
            ...
            current_packet = servers[server_id]
            if current_packet is None or current_packet.id != packet_id:
                raise RuntimeError("Departure event does not match the packet assigned to the server.")
            ...
            if queue:
                next_packet = queue.popleft()
                servers[server_id] = next_packet
                event_counter = start_service(next_packet, event_time, rng, scenario["mu_rate"], server_id, event_heap, event_counter)
...
def save_outputs(replication_rows, occupancy_rows, sample_rows, scenarios):
    """Write all required result files for the M/M/m queue.
    ...
    config = {
        ...
        "base_seed": BASE_SEED,
        "scenarios": scenarios,
        "description": "Fila M/M/m com dois servidores, buffer infinito, chegadas Poisson, servico exponencial e disciplina FCFS.",
    }
...
def main():
    """Run every scenario and replication for the infinite-buffer M/M/m queue.
    ...
    Raises:
        RuntimeError: If a departure event references the wrong server state.
